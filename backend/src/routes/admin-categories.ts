...
// GET /api/admin/categories - List categories with article counts
...
router.post('/', requireAdmin, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO categories (name, slug, color, meta_title, meta_description, meta_keywords) 
...
router.put('/:id', requireAdmin, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE categories 
       SET name=$1, slug=$2, color=$3, meta_title=$4, meta_description=$5, meta_keywords=$6 
